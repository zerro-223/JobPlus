    ...
    <el-col :span="16">
      <el-card>
        ...
        <div style="margin-top:30px">
          ...
          <el-button :type="job.isFavorited?'warning':''" @click="toggleFavorite" :disabled="!$global.user">{{ job.isFavorited?'已收藏':'收藏' }}</el-button>
        </div>
      </el-card>
    </el-col>
    <el-col :span="8">
...
export default {
  ...
  computed: {
    canDeliver() { return this.$global.user && this.$global.user.role === 1; }
  },
  mounted() { this.fetchDetail(); },
  methods: {
    fetchDetail() {
      this.$api.get('/positions/'+this.$route.params.id).then(r => this.job = r.data);
    },
    deliver() {
      if (!this.$global.user) { this.$router.push('/login'); return; }
      this.resumeDialog = true;
    },
    confirmDeliver() {
      this.delivering = true;
      this.$api.post('/deliveries', null, { params: { positionId: this.job.id, resumeId: this.selectedResume } }).then(() => {
        this.$message.success('投递成功！'); this.resumeDialog = false;
